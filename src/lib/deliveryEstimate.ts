 ...
 * 주문 마감 시각(서울 기준).
 *
 * 이 시각을 넘겨 주문하면 그날 접수분에 못 들어가므로 다음날 출고가 안 된다.
 * 하루를 통째로 미는 게 아니라 후보 시작점만 하루 뒤로 옮기고, 거기서부터
 * 배송 가능 요일·정지 기간·휴무일을 건너뛰며 가장 가까운 날을 찾는다.
 ...
 * 넉넉히 돌되 무한루프는 막는다.
 *
 * 마감(17시)을 넘겼으면 내일이 아니라 모레부터 본다. 그날 접수분에 못
 * 들어가기 때문이다. 그 뒤로는 똑같이 가장 가까운 배송 가능 요일을 찾는다.
 */
